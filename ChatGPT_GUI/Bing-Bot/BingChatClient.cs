...
namespace ChatGPT_GUI.Bing_Bot;
...
public class BingChatClient
{
    ...
    public delegate void ReceiveChangedDelegate(object sender, NetworkFixedResponse args);

    private ReceiveChangedDelegate ReceiveChangedHandle = null;

    public event ReceiveChangedDelegate ReceiveChanged
    {
        add { ReceiveChangedHandle += value; }
        remove { ReceiveChangedHandle -= value; }
    }
    ...
    private async Task<string> Receive(WebSocket ws, CancellationToken cancellationToken)
    {
        ...
        while (ws.State == WebSocketState.Open && !cancellationToken.IsCancellationRequested)
        {
            ...
            if(ReceiveChangedHandle !=null)
                ReceiveChangedHandle.Invoke(this, response);
...
public class GetMessageArgs : EventArgs
{
    public NetworkFixedResponse Source { get; set; }
}